...
SCHEMA=$(docker exec yape-notifier-db-prod psql -U postgres -d yape_notifier -t -c "\d devices" | grep -E "(user_id|commerce_id)")
...
DEVICE_STATS=$(docker exec yape-notifier-db-prod psql -U postgres -d yape_notifier -t -c "
SELECT 
    COUNT(*) as total,
...
if [ "$SIN_COMERCIO" -gt 0 ]; then
    ...
    case $REPLY in
        ...
        2)
            ...
            UPDATED=$(docker exec yape-notifier-db-prod psql -U postgres -d yape_notifier -t -c "
UPDATE devices d
SET 
...
FINAL_STATS=$(docker exec yape-notifier-db-prod psql -U postgres -d yape_notifier -t -c "
SELECT 
    COUNT(*) as total,
    COUNT(commerce_id) as con_comercio,
    COUNT(*) - COUNT(commerce_id) as sin_comercio,
